...
The session is multi-turn: a follow-up spoken code-change request while a session is already open resumes the same underlying `claude` conversation (via the `session_id` captured from the first turn) and its pending edits accumulate rather than reset, so you can layer corrections ("also rename that function") before committing anything. **Apply** and **Discard** are the only way pending edits ever reach disk — deliberately UI-only buttons, never reachable via voice/router dispatch, since writing files is the one irreversible step in this flow. Apply replays each pending edit's old→new text (or writes new files) directly against the repo; Discard drops the pending edits and leaves the repo untouched.
