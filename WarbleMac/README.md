...
A code-change intent spawns a `claude --output-format stream-json` subprocess scoped to the **Code-change repo root** configured in the text field in the window (persisted via `UserDefaults`) — point it at a scratch repo, not `warble` itself, while testing. `ClaudeCodeSession` streams the transcript live (thinking / tool use / results) into the window, and any `Edit`/`Write` tool calls it proposes are rendered as a diff below the transcript. This is deliberately view-only: Claude Code's own permission prompt for file edits is never approved by the app, so nothing is written to disk — the diff panel is built entirely from the *intended* old/new content in the streamed `tool_use` events. Multi-turn follow-ups and Apply/Discard land in a later ticket.
